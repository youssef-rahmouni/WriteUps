...
> The Python script implementing Fermat's factorization is available in this repository: https://github.com/youssef-rahmouni/WriteUps/blob/main/break-rsa/breakrsa_exploit.py
...
With the private key generated, it could be used either to answer the room's math questions (key size, factorization of n) or to authenticate over SSH:

ssh -i ~/id_rsa_key root@breakrsa.thm

### Questions Answered:
- **What is the length of the RSA key?** - Retrieved from the bit length of n in the script output
- **Factorize n?** - The script printed the values of p and q
